...
export class Record {
  ...
  constructor(winner: number, moves: Array<Array<number>>) { 
    this.winner = winner
    this.moves = moves
  }
}
...
export class Board {
  ...
  gameRecord: Array<Record> = []
  gameMoveRecord: Array<Array<number>>
  csvData = []
  starter: number = -1
  canvas: HTMLCanvasElement
  context: CanvasRenderingContext2D
  turnOne: boolean = true
  turnTwo: boolean = true
  ...
  async applyPlayerAction(player: Player, column: number): Promise<boolean> {
    ...
    //SM await this.animateAction(row, column, player.boardPiece)

    // reflect player's action to the map
    this.map[row][column] = player.boardPiece
    this.debug()
    let moveR = [row, column]
    this.gameMoveRecord.push(moveR)
   //SM await Utils.animationFrame()
    this.render()
    return true
  }
  ...
  gameDone(winner: number) {
    let done = new Record(winner, this.gameMoveRecord)
